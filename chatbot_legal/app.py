...
# # --- START: SECURE API CONFIGURATION ---
# try:
...
#     print("Successfully configured Gemini Pro model.")
# except Exception as e:
#     print(f"Error configuring Gemini Pro model: {e}")
#     model = None
# # --- END: SECURE API CONFIGURATION ---


# # Document field questions for 8 types
# DOC_FIELDS = {
#     "WILL": [
#         ("testator_name", "What is your full name (Testator)?"),
#         ("testator_address", "What is your residential address?"),
#         ("date_place", "Date and place of making the Will? (e.g., 01 Jan 2025, Mathura)"),
#         ("executor_name", "Full name and address of the Executor?"),
#         ("beneficiaries", "List beneficiaries (Name - Relation - Share%)."),
#         ("assets", "List assets/properties (Description - Location - Approx value)."),
#         ("special_clauses", "Any special instructions/conditions (Optional)."),
#         ("guardian", "Guardian name for minors (if applicable; else write 'N/A')."),
#         ("witness1", "Witness 1 - Name & Address"),
#         ("witness2", "Witness 2 - Name & Address")
#     ],
#     "FIR": [
#         ("complainant_name", "Complainant full name"),
#         ("complainant_address", "Complainant address & contact"),
#         ("date_time_of_incident", "Date & time of incident"),
#         ("incident_location", "Location of incident"),
#         ("incident_details", "Describe incident in chronological order"),
#         ("accused", "Name(s) of accused (if known) or 'Unknown'"),
#         ("witnesses", "Witness details (if any)"),
#         ("loss_details", "Loss/damage (if any)"),
#         ("sections", "Suspected sections of law (optional)"),
#         ("signature_date", "Signature date")
#     ],
#     "RTI": [
#         ("applicant_name", "Applicant full name"),
#         ("applicant_address", "Applicant address & contact"),
#         ("public_authority", "Name & address of Public Authority / PIO"),
#         ("subject_info", "Subject of information requested (short line)"),
#         ("detailed_questions", "Numbered questions / details of information required"),
#         ("period", "Period for which information is sought (from - to)"),
#         ("preferred_format", "Preferred format (Paper / Digital / Inspection)"),
#         ("fee_details", "RTI fee details (if paid) or 'N/A'"),
#         ("declaration", "Declaration (if any)"),
#         ("application_date", "Application date")
#     ],
#     "AFFIDAVIT": [
#         ("deponent_name", "Deponent full name"),
#         ("father_or_husband", "Father's / Husband's name"),
#         ("deponent_address", "Deponent residential address"),
#         ("age_dob", "Age / Date of birth"),
#         ("occupation", "Occupation"),
#         ("purpose", "Purpose of affidavit (short)"),
#         ("statements", "Statement paragraphs (numbered facts)"),
#         ("place_date", "Place & Date of oath"),
#         ("notary_details", "Notary / Oath officer details (Name, reg no.)"),
#         ("signature", "Signature of Deponent")
#     ],
#     "LEGAL_NOTICE": [
#         ("sender_name", "Sender full name / Firm name"),
#         ("sender_address", "Sender address & contact"),
#         ("recipient_name", "Recipient full name"),
#         ("recipient_address", "Recipient address & contact"),
#         ("notice_date", "Date of issuing notice"),
#         ("subject_nature", "Subject / nature of dispute"),
#         ("facts_chronology", "Chronology of facts with dates"),
#         ("legal_basis", "Legal basis / contract clause (brief)"),
#         ("demand_relief", "Specific relief / remedy demanded"),
#         ("deadline_days", "Deadline to comply (in days)")
#     ],
#     "GRIEVANCE_LETTER": [
#         ("complainant_name", "Your full name"),
#         ("complainant_address", "Your address & contact"),
#         ("designation", "Your designation / relation to organization (if any)"),
#         ("recipient_org", "Recipient organization / authority"),
#         ("recipient_designation", "Recipient designation (if known)"),
#         ("incident_date_place", "Date & place of incident"),
#         ("incident_details", "Describe grievance / incident"),
#         ("previous_steps", "Previous complaints / steps taken (if any)"),
#         ("desired_resolution", "Desired resolution or remedy"),
#         ("signature_date", "Signature & Date")
#     ],
#     "BUSINESS_LICENSE": [
#         ("applicant_name", "Applicant / Owner full name"),
#         ("business_name", "Registered business / trade name"),
#         ("business_address", "Business address"),
#         ("nature_business", "Nature / type of business activities"),
#         ("license_type", "Type of license required (Trade / Food / GST / Other)"),
#         ("duration_period", "License period requested (e.g., 1 year)"),
#         ("documents_attached", "List of documents attached (ID, proof, plan)"),
#         ("compliance_declaration", "Declaration of compliance with local regulations"),
#         ("contact_info", "Contact number & email"),
#         ("application_date", "Application date")
#     ],
#     "LEGAL_LETTER": [
#         ("sender", "Sender full name / Firm"),
#         ("recipient", "Recipient full name / Firm"),
#         ("letter_date", "Date of the letter"),
#         ("subject", "Subject / short purpose of letter"),
#         ("background_facts", "Brief factual background (dates and events)"),
#         ("legal_position", "Legal position / rights asserted"),
#         ("request_action", "Specific action or remedy requested"),
#         ("response_deadline", "Deadline for response/compliance (in days)"),
#         ("further_action", "Consequences / further legal action if ignored"),
#         ("signature_info", "Sender signature & contact details")
#     ]
# }

# # --- START: REPLACED format_draft FUNCTION ---
# # This function is now powered by the Gemini AI model.
# def format_draft(doc_type, answers):
#     if not model:
#         return "Error: Generative AI model is not configured. Please check the API key and environment variables."

#     # Create a detailed prompt for the AI model
#     prompt_details = "\n".join([f"- {key.replace('_', ' ').title()}: {value}" for key, value in answers.items()])

#     prompt = (
#         f"You are an expert legal assistant. Your task is to generate a professional legal document. "
#         f"Do not include any explanatory text, introduction, or conclusion before or after the document itself. "
#         f"The document must be based *only* on the details provided.\n\n"
#         f"**Document Type to Draft:** {doc_type}\n\n"
#         f"**Details provided by the user:**\n{prompt_details}\n\n"
#         f"Please generate the full, professionally formatted {doc_type} now."
#     )
...
#         # Add a header and disclaimer to the AI's response
#         header = f"{doc_type} - AI Generated Draft\nGenerated on {datetime.datetime.now().strftime('%d %b %Y %H:%M')}\n\n"
#         disclaimer = "\n\nDisclaimer: This draft is auto-generated by an AI model and should be reviewed by a qualified legal professional before official use."

#         return header + response.text + disclaimer

#     except Exception as e:
#         print(f"Error during API call: {e}")
#         return f"An error occurred while generating the document with the AI model: {e}"
# # --- END: REPLACED format_draft FUNCTION ---
...
#     # Greeting stage (auto-start)
#     if stage == 'GREETING':
#         session['stage'] = 'SELECT_DOC'
#         return jsonify({'type':'text','response': 'Hello 👋! I can help you draft legal documents. Which document would you like to prepare?\nOptions: Will, FIR, RTI, Affidavit, Legal Notice, Grievance Letter, Business License, Legal Letter'})

#     # Select document
#     if stage == 'SELECT_DOC':
#         doc = (user_msg or '').upper()
#         synonyms = {'RTI':'RTI','RIGHT TO INFORMATION':'RTI','WILL':'WILL','TESTAMENT':'WILL',
#                     'FIR':'FIR','POLICE':'FIR','AFFIDAVIT':'AFFIDAVIT','LEGAL NOTICE':'LEGAL_NOTICE',
#                     'NOTICE':'LEGAL_NOTICE','GRIEVANCE':'GRIEVANCE_LETTER','BUSINESS LICENSE':'BUSINESS_LICENSE',
#                     'TRADE LICENSE':'BUSINESS_LICENSE','LEGAL LETTER':'LEGAL_LETTER','LETTER':'LEGAL_LETTER','CONTRACT':'LEGAL_LETTER'}
#         doc_key = synonyms.get(doc, doc)
#         if doc_key in DOC_FIELDS:
#             session['doc_type'] = doc_key
#             session['answers'] = {}
#             session['q_index'] = 0
#             session['stage'] = 'ASK_FIELDS'
#             q = DOC_FIELDS[doc_key][0][1]
#             return jsonify({'type':'text','response': f'You selected {doc_key}. {q}'})
#         else:
#             return jsonify({'type':'text','response': 'Please choose a valid document from: Will, FIR, RTI, Affidavit, Legal Notice, Grievance Letter, Business License, Legal Letter'})
...
#         if q_index < len(fields):
#             field_key = fields[q_index][0]
#             session['answers'][field_key] = user_msg

#         q_index += 1
#         session['q_index'] = q_index

#         if q_index < len(fields):
#             next_q = fields[q_index][1]
#             return jsonify({'type':'text','response': next_q})
#         else:
#             session['stage'] = 'GENERATE'
#             return jsonify({'type':'text','response':'All information received. Generating your draft... (this may take a moment)'})

#     # Generate draft
#     if stage == 'GENERATE':
#         doc_type = session.get('doc_type','LEGAL_LETTER')
#         answers = session.get('answers',{})
#         draft = format_draft(doc_type, answers) # This now calls the Gemini model
...
#     if not draft:
#         return "No draft available. Please create a document first.", 400
...
#     # Note: This path might need adjustment depending on your deployment environment
#     out_path = os.path.join(os.getcwd(), 'generated_document.pdf')
